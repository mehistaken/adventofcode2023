...
const findNumber = (y, x) => {
  if (y >= map.length || x >= map[0].length) return 0;
  ...
  if (char.match(/\d/) != null) {  
    ...
    while (char.match(/\d/) != null && start > 0) {
      start--;
      char = map[y][start];
    }
    if (char == '.' && start >= 0) start++;
    
    char = map[y][end];
    
    while (char.match(/\d/) != null && end < map[y].length - 1) {
      end++;
      char = map[y][end];
    }
    if (end < map[y].length - 1) end--;

    let len = end - start + 1;

    return ~~map[y].splice(start, len, ...Array(len).fill('.')).join('');
  }

  return 0;
}

let ans = map.reduce((total, row, y) => {
  return total + row.reduce((subtotal, col, x) => {
    let sum = 0;
    
    if (col.match(/[^\d.]/) != null) {
      sum += findNumber(y-1, x-1);
      sum += findNumber(y-1, x);
      sum += findNumber(y-1, x+1);
      sum += findNumber(y, x-1);
      sum += findNumber(y, x+1);
      sum += findNumber(y+1, x-1);
      sum += findNumber(y+1, x);
      sum += findNumber(y+1, x+1);
