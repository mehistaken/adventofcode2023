...
let ans = games.reduce((total, game) => {
  let [id, pulls] = game.split(":");
  id = ~~id.match(/Game (\d{1,3})/)[1];
  
  let res = pulls.split(';').map(pull => {
    
    let possible = pull.split(',').map(ea => {
      let [n, c] = ea.trim().split(' ');
      n = ~~n;
      
      return n <= max[c];
    });

    return possible.indexOf(false) == -1;
  });
  
  return total += res.indexOf(false) == -1 ? id : 0;
}, 0);
...
const sample2 = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue`;

const games2 = input.split(/\n/);


let ans2 = games2.map(game => {
  let [id, pulls] = game.split(": ");
  id = ~~id.match(/Game (\d{1,3})/)[1];
  ...
  pulls.split(/[,;]/g).map(ea => {
    let [n, c] = ea.trim().split(' ');
    n = ~~n;

    min[c] = min[c] == null || n > min[c] ? n : min[c];
  });
